...
public class Client extends Thread{
    ...
    public String obtainRequest(String requestFilePath){
        String request = "";     
                ...
                line = line.trim();
                    if (!line.isEmpty()) {
                        ...
                        if (massages[0] == "PUT") {
                            ...
                            request = String.format("%s P %s %s",lengthCode, massages[1], value);
                        } else if (massages[0] == "READ") {
                            String lengthCode = String.format("%03d", String.format("000 R %s", massages[1]).length());
                            request = String.format("%s R %s", lengthCode, massages[1]);
                        } else if (massages[0] == "GET") {
                            String lengthCode = String.format("%03d", String.format("000 G %s", massages[1]).length());
                            request = String.format("%s G %s", lengthCode, massages[1]);
                        } else {
